...
function useBroadcastAudio() {
  ...
  const start = React.useCallback(async () => {
    ...
    try {
      const resp = await fetch('assets/eas-tone.wav');
    ...
    try {
      const resp = await fetch('assets/click.wav');
      const arr = await resp.arrayBuffer();
      const buf = await ctx.decodeAudioData(arr);
    ...
    try {
      const audioEl = new Audio('assets/inkspots-its-all-over.mp3');
  ...
  const update = React.useCallback((time, playing) => {
    ...
    // Ink Spots playback: 250s through end of broadcast (348s)
    const ink = inkAudioRef.current;
    const inkG = inkGainRef.current;
    if (ink && inkG) {
      if (time >= 250 && time < 363) {
        ...
        if (time > 359) vol = 0.7 * Math.max(0, 1 - (time - 359) / 4);
        inkG.gain.setTargetAtTime(vol, now, 0.2);
      } else if (inkStartedRef.current && (time < 249.5 || time >= 363)) {
...
function AudioEngine() {
  ...
  const [armBlink, setArmBlink] = React.useState(true);

  // Blink the "click to receive" line
  React.useEffect(() => {
    if (received) return;
    const id = setInterval(() => setArmBlink(b => !b), 600);
  ...
  if (received) {
    // After receive: tiny re-arm/audio button (fallback if audio context didn't init)
    return (
      <button
        onClick={start}
        title="Re-enable audio"
        style={{
          position: 'absolute',
          bottom: 12, right: 12,
          zIndex: 100,
  ...
  // Pre-roll: incoming alert overlay
  return (
    <div
    ...
    >
      ...
      <div style={{
        ...
        fontSize: 18, letterSpacing: '0.32em', color: '#7a8270',
        borderBottom: '1px solid rgba(212,216,200,0.12)',
      }}>
        <span>CH 03</span>
        <span style={{ color: '#e85a3a' }}>● LIVE</span>
        <span>OPEN FEED</span>
      </div>
      ...
      <div style={{
        ...
        textAlign: 'center',
        padding: '14px 0',
        fontSize: 22, letterSpacing: '0.34em',
        marginBottom: 36,
        animation: 'csspulse 1.4s ease-in-out infinite',
        boxShadow: '0 0 24px rgba(232,90,58,0.4)',
      }}>
        ▲ EMERGENCY ALERT SYSTEM ▲
      </div>

      <style>{`
        @keyframes csspulse { 0%,100% { opacity: 1; } 50% { opacity: 0.55; } }
      `}</style>
      ...
      <div style={{
        ...
        textAlign: 'center',
        textShadow: '0 0 18px rgba(212,216,200,0.25)',
      }}>
      ...
      <div style={{
        ...
        AUTHORITY: CIVIL SAFETY SERVICE<br/>
        ARCHIVE NO. 03 · DURATION 5:00<br/>
      ...
      <div style={{
        marginTop: 64,
        padding: '14px 38px',
        border: `2px solid ${hover ? '#e85a3a' : '#d4d8c8'}`,
        color: hover ? '#e85a3a' : '#d4d8c8',
        fontSize: 28, letterSpacing: '0.4em',
        opacity: armBlink ? 1 : 0.35,
        transition: 'opacity 0.1s, color 0.15s, border-color 0.15s',
      }}>
        ▶ CLICK TO RECEIVE
      </div>
      ...
      <div style={{
        ...
        textAlign: 'center',
        fontSize: 14, letterSpacing: '0.36em',
        color: '#5a604f',
      }}>
        AUDIO WILL BE ENABLED · USE HEADPHONES IF AVAILABLE
      </div>
    </div>
  );
}
